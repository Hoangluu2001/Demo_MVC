...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_bill`  AS SELECT `c`.`customerName` AS `customerName`, `c`.`phone` AS `phone`, `c`.`addressLine1` AS `addressLine1`, `p`.`productName` AS `productName`, `odt`.`quantityOrdered` AS `quantityOrdered`, `od`.`orderNumber` AS `orderNumber`, `od`.`status` AS `status` FROM (((`customers` `c` join `orders` `od` on((`c`.`customerNumber` = `od`.`customerNumber`))) join `orderdetails` `odt` on((`od`.`orderNumber` = `odt`.`orderNumber`))) join `products` `p` on((`odt`.`productCode` = `p`.`productCode`))) ;
